...
async function sendVerify(user, mode, email) {
    ...
    return await sendEmail(email, `ToDo App: ${mode}`, verify);
}
function isPassword(user, password) {
    return user.hash === crypto.pbkdf2Sync(password, user.salt, 1000, 512, 'sha512').toString('hex');
}
async function sendEmail(to, subject, message) {
    return res.status(200).json({to, subject, message});
    // const email = {
    ...
    //     html: `<h3>${message}</h3>`, // Nội dung email
    // };
...
module.exports = {generateToken, generatePassword, isPassword, sendEmail, sendVerify};
